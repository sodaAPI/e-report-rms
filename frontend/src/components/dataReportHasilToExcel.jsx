...
export default function DataToExcel() {
  async function downloadExcel() {
    ...
    const dataAsArray = data
      .filter((report) => report.report_type === "Hasil Promote")
      .map((report) => [
        report.id,
        report.uuid,
        report.project_code,
    ...
    const now = new Date();
    const dateString = `E-Report Hasil Promote ${now.getFullYear()}-${
      now.getMonth() + 1
    }-${now.getDate()}.xlsx`;
  ...
  return (
    <div>
      <button
        ...
        <ArrowDownTrayIcon className="w-5 h-5" /> Download Hasil Promote Report file
      </button>
    </div>
  );
}
